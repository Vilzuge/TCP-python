...
def handle(client):
    while True:
        try:
            ...
            #broadcast message
            broadcast(user_header + user_content + channel_header + channel_content 
                + msg_header + msg_content, client)
            ...
            if (msg_content.decode("utf-8") == "/q"):
                ...
                ch = channel_content.encode("utf-8")
                channel_header = f'{len(ch):<{HEADER}}'.encode("utf-8")

                broadcast(user_header + user + channel_header + ch 
                    + header_bye + msg_bye, client)
...
def receive():
    while True:
        ...
        try:
            ...
            ask_channel = "server".encode("utf-8")
            channel_header = f'{len(ask_channel):<{HEADER}}'.encode("utf-8")

            client.send(user_header + user 
                + channel_header + ask_channel + nick_header + nick)
            ...
            channel_header = client.recv(HEADER)
            channel_length = int(channel_header.decode('utf-8').strip())
            channel = client.recv(channel_length).decode('utf-8')
            ...
            print(f'New user entered {channel} with the nickname: {nickname}')
            user = 'SERVER'.encode('utf-8')
            user_header = f"{len(user):<{HEADER}}".encode('utf-8')
            ch = channel.encode("utf-8")
            channel_header = f'{len(ch):<{HEADER}}'.encode("utf-8")
            msg_welcome = f'{nickname} joined ({channel}))!'.encode('utf-8')
            header_welcome = f"{len(msg_welcome):<{HEADER}}".encode("utf-8")

            broadcast(user_header + user + channel_header + ch 
                + header_welcome + msg_welcome, client) #broadcast join event
